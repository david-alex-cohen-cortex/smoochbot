...
function Smoochbot(configuration) {
  ...
  smooch_botkit.createWebhookEndpoints = function(webserver) {
    ...
    smooch_botkit.log('** Serving webhook endpoints for Smooch events at: ' + 
      `http://MY_HOST:${smooch_botkit.config.port}/${smooch_botkit.config.incoming_webhook}`);
    webserver.post(smooch_botkit.config.incoming_webhook, function(req,res) {
